...
function copy(member) {
	...
	return member(member.user, member.fname, member.lname, member.year);
}
...
function one(user) {
	if(typeof user !== 'string'){
		return result(false, 'invalid input, user name must be a string', []);
	}
	else{
		var member = find(user);
		if(member){
			return result(true, 'team member found', copy(member));
		}
		else{
