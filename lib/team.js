...
var team = [
	...
	member('ktfarr', 'Kevin', 'Farr', 'senior'),
	member('nefuller', 'Nate', 'Fuller', 'senior'),
	member('cogiang', 'Co', 'Giang', 'senior'),
	member('dlazier', 'Dan', 'Lazier', 'senior'),
	member('abradley', 'Adam', 'Bradley', 'junior'),
	member('zhili', 'Zhi', 'Li', 'senior')
];
 ...
 */
function copy(teamMember) {

	var copy = member(teamMember.user, teamMember.fname, teamMember.lname, teamMember.year)
...
function one(user) {
	...
	if(typeof user !== "string"){
		resObject = result(false, 'invalid input, user name must be a string', []);
	}
	else{
		var member = find(user);
		if(member){
			var data = copy(member);
			resObject = result(true, 'team member found', [data]);
		}
		else{
			resObject = result(false, 'team member not found', []);
		}
	}
	return resObject;
}
